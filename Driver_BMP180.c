...
static int bmp180_calculate_temperature(int raw_temp) {
    ...
    printk(KERN_INFO "Calculated temperature: %d.%d (°C)\n", temperature / 10, abs(temperature % 10));
    return temperature;  // Trả về nhiệt độ thực tế
}

static int32_t bmp180_calculate_pressure(int32_t raw_press, int32_t B5) {
    ...
    X3 = X1 + X2;
    B3 = (((calib_data.AC1 * 4 + X3) << 0) + 2) >> 2;  // << 0 vì oss = 0
    ...
    B7 = ((uint32_t)(raw_press - B3)) * 50000;
    ...
    printk(KERN_INFO "Calculated pressure: %d (Pa)\n", p);
    return p;
}
